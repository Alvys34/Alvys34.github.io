(() => {
  ...
  const navItems = [["Home", "index.html", "home"], ["Portfolio", "portfolio/index.html", "portfolio"], ["Blog", "blog/index.html", "blog"], ["About", "about/index.html", "about"], ["Resume", "resume/index.html", "resume"], ["Contact", "contact/index.html", "contact"]];
  ...
  if (projectId) {
    ...
    if (project) {
      ...
      document.querySelector("[data-detail-period]").textContent = project.period;
      ...
      document.querySelector("[data-detail-highlights]").innerHTML = project.highlights.map((item) => `<li>${item}</li>`).join("");
      const image = document.querySelector("[data-detail-image]");
      const imageSection = document.querySelector("[data-detail-image-section]");
      if (project.image) image.src = project.image; else imageSection.hidden = true;
      const related = document.querySelector("[data-detail-related]");
      if (project.relatedPost) { const post = content.posts.find((item) => item.id === project.relatedPost); related.href = postUrl(post); related.textContent = `${post.title} ↗`; } else related.hidden = true;
    }
  }
  const postId = document.body.dataset.post;
  if (postId) { const post = content.posts.find((item) => item.id === postId); if (post) { document.title = `${post.title} — ${content.site.name}`; document.querySelector("[data-detail-title]").textContent = post.title; document.querySelector("[data-detail-role]").textContent = `${post.type} · ${post.date}`; document.querySelector("[data-detail-summary]").textContent = post.summary; document.querySelector("[data-detail-body]").textContent = post.body; } }
})();
